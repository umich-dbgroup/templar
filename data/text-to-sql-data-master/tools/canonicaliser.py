...
def subquery_range(current, pos, tokens):
    if tokens[pos] == '(':
        start = pos
        end = pos + 1
        depth = 1
...
def standardise_aliases(query, schema):
    ...
    for i, word in enumerate(tokens):
        current_subquery = subquery_range(current_subquery, i, tokens)
        if word == "FROM":
            seen_from[current_subquery] = True
        elif current_subquery not in seen_from:
            seen_from[current_subquery] = False
        if word in schema[0]:
            ...
            if tokens[i-2] not in schema[0]:
                word = "DERIVED_TABLE" if seen_from[current_subquery] else "DERIVED_FIELD"
                count[word] = count.get(word, -1) + 1
                alias = word + "alias"+ str(count[word])
    ...
    for i, word in enumerate(tokens):
        ...
        if len(parts) == 2:
            if (current_subquery[0], parts[0]) in aliases:
                ...
                field = parts[1]
                if (current_subquery[0], parts[1]) in aliases:
                    field = aliases[current_subquery[0], parts[1]]
...
###                    print("Found", i, word, table, current_subquery)
                    for pair in aliases:
                        if pair[0] == current_subquery[0] and pair[1] == table:
                            alias = aliases[pair]
                            tokens[i] = alias +'.' + word
                            done = True
                            break
        ...
        for info in to_rearrange:
            for i in range(info[1], info[2]+1):
                for j in range(chunks[i][0], chunks[i][1]+1):
                    ...
                    if token != separator or (chunks[i][1] - chunks[i][0]) > 1:
...
def order_sequence(tokens, start, end, variables):
    ...
    chunks = [(start, start)]
    while cpos < end:
        sub = subquery_range(None, cpos, tokens)
        if sub is None:
...
def make_canonical(query, schema, variables):
...
def standarise_file(filename, schema_filename, log, overwrite):
    ...
    with open(filename, 'r') as input_file:
        ...
        for query in data['sql']:
            if log:
                print(query)
            nqueries.append(make_canonical(query, schema, set()))
            if log:
                print(nqueries[-1])
                print()
        data['sql'] = nqueries
        ...
        if 'sql-with-vars' in data:
            ...
            if log:
                print(query)
            query = make_canonical(query, schema, variables)
            if log:
                print(query)
                print()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--test', help='Run test cases and exit.', action='store_true')
    parser.add_argument('--log', help='Print SQL before and after.', action='store_true')
    parser.add_argument('--schema', help='The schema for these queries.', required=True)
    parser.add_argument('--overwrite', help='Replace the file rather than creating a new one.', action='store_true')
    args = parser.parse_args()

    if not args.test:
        for line in sys.stdin:
            standarise_file(line.strip(), args.schema, args.log, args.overwrite)
    else:
        sample_queries = [
